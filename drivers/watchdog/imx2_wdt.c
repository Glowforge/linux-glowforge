 ...
 *
 * Support for device tree defaults and panic-on-pretimeout is
...
MODULE_PARM_DESC(pretimeout, "Watchdog pretimeout in seconds (default=0)");

static bool panic_on_pretimeout = 0;
module_param(panic_on_pretimeout, bool, 0);
MODULE_PARM_DESC(panic_on_pretimeout, "Panic if pretimeout expires "
	"(default=false)");
...
static irqreturn_t imx2_wdt_isr(int irq, void *dev_id)
{
	...
	if (val & IMX2_WDT_WICR_WTIS) {
		...
		__raw_writew(val, imx2_wdt.base + IMX2_WDT_WICR);
		if (!panic_on_pretimeout) {
			printk(KERN_INFO \
				"watchdog pre-timeout:%d, "
				"%d Seconds remained\n",
				imx2_wdt.pretimeout,
				imx2_wdt.timeout-imx2_wdt.pretimeout);
		} else {
			imx2_wdt_pretimeout_panic();
		}
	}
	return IRQ_HANDLED;
}
...
static int imx2_wdt_open(struct inode *inode, struct file *file)
{
	...
	imx2_wdt.opened_pid = current->pid;
	imx2_wdt_start();
...
static int __init imx2_wdt_probe(struct platform_device *pdev)
{
	...
	/* Get panic-on-pretimeout flag from dt */
	panic_on_pretimeout = panic_on_pretimeout ||
		of_property_read_bool(pdev->dev.of_node,
			"fsl,panic-on-pretimeout");

	/* Get optional default timeout from dt */
	of_property_read_u32(pdev->dev.of_node, "fsl,default-timeout",
		&timeout);
	...
	/* Get optional pretimeout default from dt */
	of_property_read_u32(pdev->dev.of_node, "fsl,default-pretimeout",
		&pretimeout);
	imx2_wdt.pretimeout = clamp_t(unsigned, pretimeout, 0, timeout);
	if (imx2_wdt.pretimeout != pretimeout) {
		dev_warn(&pdev->dev, "Initial pretimeout out of range! "
			"Clamped from %u to %u\n",
			pretimeout, imx2_wdt.pretimeout);
	}
	if (imx2_wdt.pretimeout) {
		imx2_wdt_set_pretimeout(imx2_wdt.pretimeout);
	}

	dev_info(&pdev->dev,
		"IMX2+ Watchdog Timer enabled. timeout=%ds, "
		"pretimeout=%ds, "
		"(nowayout=%d, panic_on_pretimeout=%d)\n",
		imx2_wdt.timeout, imx2_wdt.pretimeout, nowayout,
		panic_on_pretimeout);
