...
static void sdma_tasklet(unsigned long data)
{
	...
	if (sdmac->flags & (IMX_DMA_SG_LOOP|IMX_DMA_CUSTOM_CALLBACK))
...
static irqreturn_t sdma_int_handler(int irq, void *dev_id)
{
	...
	while (stat) {
		...
		if (sdmac->status == DMA_IN_PROGRESS ||
			(sdmac->flags & (IMX_DMA_SG_LOOP|IMX_DMA_CUSTOM_CALLBACK)))
		...
		void *cb_param)
{
	...
	if (int_cb)
		...
		sdmac->flags &= ~IMX_DMA_CUSTOM_CALLBACK;
	sdmac->desc.callback = int_cb;
	sdmac->desc.callback_param = cb_param;
	spin_unlock_irqrestore(&sdmac->lock, flags);
}
